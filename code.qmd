...
### Número de tubérculos con zebra chip

```{r}
datos_ntz <- data %>% filter(!is.na(ntz))

summary(datos_ntz$ntz)

modelo_ntz <- glm(ntz ~ Tipo.de.control + block,
                  family = poisson(link = "log"),
                  data = datos_ntz)

anova(modelo_ntz, test = "Chisq")

Anova(modelo_ntz, type = 3)

# Calcular dispersión: si >>1, hay sobredispersión

deviance(modelo_ntz) / df.residual(modelo_ntz)

modelo_ntz_quasi <- glm(ntz ~ Tipo.de.control + block,
                        family = quasipoisson(link = "log"),
                        data = datos_ntz)

Anova(modelo_ntz_quasi, type = 3)

anova(modelo_ntz_quasi, test = "Chisq")

mc <- emmeans(modelo_ntz_quasi, ~ Tipo.de.control, type = "response") %>%
  cld(Letters = letters, reversed = T) %>% 
  mutate(across(".group", ~ trimws(.))) %>% 
  rename(sig1 = ".group")
...
### Número de tubérculos sanos

```{r}
datos_nts <- data %>% filter(!is.na(nts))

modelo_nts <- glm(nts ~ Tipo.de.control + block,
                  family = poisson(link = "log"),
                  data = datos_nts)
...
deviance(modelo_nts) / df.residual(modelo_nts)

modelo_nts_quasi <- glm(nts ~ Tipo.de.control + block,
                        family = quasipoisson(link = "log"),
                        data = datos_nts)

Anova(modelo_nts_quasi, type = 3)

anova(modelo_nts_quasi, test = "Chisq")

mc <- emmeans(modelo_nts_quasi, ~ Tipo.de.control, type = "response") %>%
  cld(Letters = letters, reversed = T) %>% 
  mutate(across(".group", ~ trimws(.))) %>% 
  rename(sig1 = ".group")
